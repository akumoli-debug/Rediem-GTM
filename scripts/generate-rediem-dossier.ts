...
async function main() {
  ...
  const dossier = JSON.parse(raw) as unknown;
  const markdown = generateRediemDossier({ dossier: dossier as never });

  await mkdir(dirname(args.output), { recursive: true });
  await writeFile(args.output, `${markdown.trimEnd()}\n`, "utf8");
  console.log(`Wrote Rediem dossier memo to ${args.output}`);
}
